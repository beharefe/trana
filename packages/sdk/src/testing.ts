/**
 * Test utilities for Trana Guard — Node.js / Anchor test environments.
 *
 ...
 *   2. Register it in the onchain registry PDA
 *   3. Build a valid secp256r1 proof instruction for any TranaIntent
...
import { hashIntent } from "./react/intent"
import type { TranaIntent } from "./react/intent"
...
export type TestPasskeyHandle = {
  ...
  privKey: Uint8Array
  /** Arbitrary credential ID — pass to register_two_fa */
 ...
 *   - clientDataJSON: {"type":"webauthn.get","challenge":"<base64url(hashIntent)>",...}
 *
 ...
 *
 * This is the test equivalent of useTrana().authorizeAndSend().
 *
 * @param intent    — a frozen TranaIntent (build with buildIntent())
...
export function buildTestProofIx(
  ...
  // 1. Challenge = intent hash (same as SDK's authorizeAndSend)
  const challenge = hashIntent(intent)

  // 2. Minimal authenticatorData (37 bytes: rpIdHash + flags + counter)
  //    Structurally valid per WebAuthn spec §6.1
  const rpIdHash   = sha256Bytes(new TextEncoder().encode(rpId))
  const authData   = new Uint8Array(37)
  authData.set(rpIdHash, 0)  // rpIdHash: bytes 0–31
  authData[32] = 0x05        // flags: UP (bit 0) + UV (bit 2) — user present + verified
  // signCount bytes 33–36: 0000 (acceptable for tests)

  // 3. clientDataJSON embeds the challenge as base64url
  ...
  // 4. ECDSA e-value = SHA-256(authenticatorData ‖ SHA-256(clientDataJSON))
  //    This is what buildWebAuthnMessage() computes and what the authenticator signs.
  const eValue = buildWebAuthnMessage(authData, clientDataJSON)  // 32 bytes
 ...
 * but call it anyway to match production behavior.
 */
