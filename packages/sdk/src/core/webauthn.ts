 ...
 *   Node.js       — IN PROGRESS: use `@beharefe/sdk/testing` for Node test environments
...
function assertWebAuthnAvailable(): void {
  if (typeof navigator === "undefined" || !navigator.credentials?.create) {
    // IN PROGRESS: Node.js support — use `@beharefe/sdk/testing` for Node test environments
    throw new TranaWebAuthnError(
      "unavailable",
      "WebAuthn not available. Run in a browser environment. " +
      "For Node.js / test environments, use @beharefe/sdk/testing instead.",
    )
  }
}
