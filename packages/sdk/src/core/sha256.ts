...
/**
 * Pure-JS SHA-256 (RFC 6234) — zero dependencies, works in browser and Node.
 *
 * Used for intent hashing, SPKI parsing, and secp256r1 message construction.
 * This is intentionally a self-contained implementation: no subtle.crypto
 * polyfills needed, no dynamic imports, tree-shakes to ~2 KB.
 */

/** SHA-256 over raw bytes. Returns 32-byte Uint8Array. */
export function sha256Bytes(input: Uint8Array): Uint8Array {
  return pureSha256(input)
}

/** SHA-256 over a UTF-8 string. Returns 32-byte Uint8Array. */
export function sha256(input: string): Uint8Array {
  return pureSha256(new TextEncoder().encode(input))
}

/** Read first 8 bytes of a params buffer as little-endian u64. */
