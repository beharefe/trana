...
The SDK handles the rest: simulates the transaction to detect which policy fires, lazily registers the passkey on first use, shows a confirmation modal, prompts Touch ID / YubiKey, constructs the secp256r1 + record_proof instructions, and retries on blockhash expiry.
