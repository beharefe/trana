...
The expected cluster (`"mainnet-beta"`, `"devnet"`, `"localnet"`) is a compile-time constant baked into the program binary via Cargo feature flags — not stored in a mutable config account. A proof signed for devnet will fail with `ClusterMismatch` when submitted to a mainnet binary, even if both binaries share the same program ID.
